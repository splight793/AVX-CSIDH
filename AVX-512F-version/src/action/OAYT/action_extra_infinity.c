...
// look up the secret exponent in constant time (from [CVCCD+19] code)
...
// will be executed sequentially 8 times. (from [CVCCD+19] code)
static void action_1w(proj C, const uint8_t *sk, const proj A)
{
...
static void action_8x1w(htpoint_t C, __m512i* e, const __m512i *sk, const htpoint_t A)
{
  ...
  // put these primes directly in the complement of the BA 
  compba[0][sicoba[0]] = ba[0][22];
  sicoba[0] += 1;
  compba[1][sicoba[1]] = ba[1][22];
  sicoba[1] += 1;
  compba[2][sicoba[2]] = ba[2][22];
  sicoba[2] += 1;
  compba[2][sicoba[2]] = ba[2][21];
  sicoba[2] += 1;
  compba[1][sicoba[1]] = ba[1][21];
  sicoba[1] += 1;
  compba[0][sicoba[0]] = ba[0][21];
  sicoba[0] += 1;
  compba[2][sicoba[2]] = ba[2][20];
  sicoba[2] += 1;

  // perform HT_NUMISO isogeny computations in the batched component
  while (total < HT_NUMISO) {
    m = (m+1) % numba;                  // public parameter SIMBA index

    if (count == ROUND*NUMBA) {         // merge the subsets when required rounds finished
      m = 0;                    
      sicoba[0] = 0;          
      sizeba[0] = 0;
      numba = 1;
      ...
      if (fnsh[ba[m][i]]) continue;
        ...
        if (!f_inf) {
          ...
          if (ba[m][i] != lastiso[m]) {
            ...
            point_cswap_8x1w(&T0, &T2, VXOR(bc, vone));
            point_cswap_8x1w(&T1, &T3, VXOR(bc, vone));
          }
          point_cmove_8x1w(&A0, &A1, VXOR(bc, vone));
...
void action(htpoint_t C, const __m512i *sk, const htpoint_t A)
{
  ...
  for (i = 0; i < 8; i++) action_1w(llC[i], lle[i], llA[i]);
