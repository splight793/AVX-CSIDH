...
void test_action()
{
  ...
  // have been verified by [CVCCD+19] code
  ...
  htpoint vPK, vSS, vE;
  htfe_t t0, t1;
  uint64_t pk_a[HT_NWORDS], pk_b[HT_NWORDS], pk_c[HT_NWORDS], pk_d[HT_NWORDS];
  uint64_t pk_e[HT_NWORDS], pk_f[HT_NWORDS], pk_g[HT_NWORDS], pk_h[HT_NWORDS];
  uint64_t ss_a[HT_NWORDS] = { 0 }, ss_b[HT_NWORDS] = { 0 };
  uint64_t ss_c[HT_NWORDS] = { 0 }, ss_d[HT_NWORDS] = { 0 };
  uint64_t ss_e[HT_NWORDS] = { 0 }, ss_f[HT_NWORDS] = { 0 };
  uint64_t ss_g[HT_NWORDS] = { 0 }, ss_h[HT_NWORDS] = { 0 };
  uint64_t r52[HT_NWORDS], r64[HT_NWORDS];
  int i;
  ...
  mpi_print("  - B : ", r64, 8);
  mpi_conv_52to64(r64, pk_c, HT_NWORDS, HT_NWORDS);
  mpi_print("  - C : ", r64, 8);
  mpi_conv_52to64(r64, pk_d, HT_NWORDS, HT_NWORDS);
  mpi_print("  - D : ", r64, 8);
  mpi_conv_52to64(r64, pk_e, HT_NWORDS, HT_NWORDS);
  mpi_print("  - E : ", r64, 8);
  mpi_conv_52to64(r64, pk_f, HT_NWORDS, HT_NWORDS);
  mpi_print("  - F : ", r64, 8);
  mpi_conv_52to64(r64, pk_g, HT_NWORDS, HT_NWORDS);
  mpi_print("  - G : ", r64, 8);
  mpi_conv_52to64(r64, pk_h, HT_NWORDS, HT_NWORDS);
  mpi_print("  - H : ", r64, 8);

  // We simply swap the position of (pk_a and pk_b), (pk_c and pk_d),
  // (pk_e and pk_f), (pk_g and pk_h) in the vectors. 
  ...
  mpi_print("  - B : ", r64, 8);
  mpi_conv_52to64(r64, ss_c, HT_NWORDS, HT_NWORDS);
  mpi_print("  - C : ", r64, 8);
  mpi_conv_52to64(r64, ss_d, HT_NWORDS, HT_NWORDS);
  mpi_print("  - D : ", r64, 8);
  mpi_conv_52to64(r64, ss_e, HT_NWORDS, HT_NWORDS);
  mpi_print("  - E : ", r64, 8);
  mpi_conv_52to64(r64, ss_f, HT_NWORDS, HT_NWORDS);
  mpi_print("  - F : ", r64, 8);
  mpi_conv_52to64(r64, ss_g, HT_NWORDS, HT_NWORDS);
  mpi_print("  - G : ", r64, 8);
  mpi_conv_52to64(r64, ss_h, HT_NWORDS, HT_NWORDS);
  mpi_print("  - H : ", r64, 8);

  puts("\n* Correctness:");
  if (memcmp(ss_a, ss_b, HT_NWORDS*sizeof(uint64_t)))
    printf("Shared secrets between A and B : \x1b[31mNOT EQUAL!\x1b[0m\n");
  else
    printf("Shared secrets between A and B : \x1b[32mEQUAL!\x1b[0m\n");

  if (memcmp(ss_c, ss_d, HT_NWORDS*sizeof(uint64_t)))
    printf("Shared secrets between C and D : \x1b[31mNOT EQUAL!\x1b[0m\n");
  else
    printf("Shared secrets between C and D : \x1b[32mEQUAL!\x1b[0m\n");

  if (memcmp(ss_e, ss_f, HT_NWORDS*sizeof(uint64_t)))
    printf("Shared secrets between E and F : \x1b[31mNOT EQUAL!\x1b[0m\n");
  else
    printf("Shared secrets between E and F : \x1b[32mEQUAL!\x1b[0m\n");

  if (memcmp(ss_g, ss_h, HT_NWORDS*sizeof(uint64_t)))
    printf("Shared secrets between G and H : \x1b[31mNOT EQUAL!\x1b[0m\n");
  else
    printf("Shared secrets between G and H : \x1b[32mEQUAL!\x1b[0m\n");
...
void test_multi_actions(int round)
{
  ...
  for (j = 0; j < round; j++) {
    ...
    wrong |= memcmp(ss_e, ss_f, HT_NWORDS*sizeof(uint64_t));
    wrong |= memcmp(ss_g, ss_h, HT_NWORDS*sizeof(uint64_t));
